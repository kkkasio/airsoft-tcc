<div class="form-group">
    ...
    <input id="name" type="text" name="name" class="form-control @error('name') is-invalid @enderror"
        placeholder="Nome do time" value="{{ old('name') || $team->name ? $team->name : ''}}" required>
    @error('title')
...
<div class="form-group">
    ...
    <div class="input-group input-group-flat">
        ...
        <input type="text" id="slug" name="slug" value="{{ old('slug') || $team->slug ? $team->slug : '' }}"
            class="form-control @error('slug') is-invalid @enderror ps-1" autocomplete="off"
            <?php echo $team->slug ? 'disabled': '' ?> required>
...
<div class="form-group">
    ...
    <textarea id="about" class="form-control @error('about') is-invalid @enderror" name="about"
        ...
        required>{{ old('about') || $team->about ? $team->about : '' }}</textarea>
    @error('about')
    <span class="invalid-feedback" role="alert">
        <strong>{{ $message }}</strong>
    </span>
